...
\begin{tikzpicture}[
  ...
  \node (boolring) [below=1.1cm of ring] {\algnode{cRing}{Boolean Ring}{$(A,+,\cdot,0,1)$}{%
    $\forall$\ $a\cdot a=a$\\
    $\operatorname{RingWithUnity}$\\
    $\land\ \operatorname{IdempotentOperation}(\cdot,A)$}};
  ...
  \draw[isa] (ring.south) -- (boolring.north)
    node[lbl,    midway, right=3pt] {IS-A}
    node[rellbl, midway, left=3pt]  {$+$idempotence};
